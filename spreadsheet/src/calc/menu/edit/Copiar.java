...
public class Copiar extends Command<AppCalc> {
	...
	public final void execute() throws InvalidCellRange {
		...
		entity().getCutBuffer().clear(); // Cleans the cutBuffer before rewriting it
		...
		if (address.toString().contains(":")) { // It is a range
			...
			if (cell != null) {
				if (cell.getConteudo() == "") { //It means the cell has a literal content
					new Cell(_folhaCalculo, entity().getCutBuffer(), 1, cell.getValor());
				} else { //It means the cell has a content that is not literal
